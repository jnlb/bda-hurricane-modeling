...
We run this model for the same number of iterations as before: 4000 using with 2000 as warm-up, but in this case we also increased the maximum tree depth to 15. Convergence was also reached in this case, as all $\hat{R}$ values approximated to 1.0 and all ESS were higher than 100.
...
To execute this model we used 4000 iterations with half of them as warm-up and a maximum tree depth of 15. We checked the $\hat{R}$ values and all of them approximated to 1.0, so we can state that the chains converged. Moreover ESS is always high enough.
...
The PSIS-LOO elpd for the first linear model is -1636.0, the effective number of parameters is 9 with looic of 3272.1 and Monte Carlo Standard Error of 0.0. All the Pareto k estimates are lower than 0.5, as we can see in Figure. X, which means that the PSIS-LOO estimate can be considered to be reliable.
